...
fn test_readonly_directory() -> Result<()> {
    ...
    assert!(result.is_err() || result.is_ok(), "Either succeeds or fails gracefully");
...
fn test_concurrent_backup_operations() -> Result<()> {
    ...
    for i in 0..5 {
        let backup_path = temp.path().join(format!("file{}.txt.autoversion.backup", i));
