...
impl VersionUpdater for GenericUpdater {
    fn get_current_version(&self, project_path: &Path) -> Result<String> {
        let version_file = self.find_version_file(project_path).ok_or_else(|| {
            anyhow!(
                "No version file found. Looked for: {:?}",
                self.version_files
            )
        })?;
    ...
    fn update_version(&self, project_path: &Path, new_version: &str) -> Result<Vec<String>> {
        ...
        let new_content = self.create_version_content(new_version, original_content.as_deref());
    ...
    fn preview_changes(
        ...
        let old_version = if version_file.exists() {
            self.get_current_version(project_path)
...
build=123"#;
    ...
    fn test_create_version_content() {
        ...
        // Preserve format
        assert_eq!(
            updater.create_version_content("1.2.3", Some("v1.0.0")),
            "v1.2.3\n"
        );
        assert_eq!(
            updater.create_version_content("1.2.3", Some("version=1.0.0")),
            "version=1.2.3\n"
        );
        assert_eq!(
            updater.create_version_content("1.2.3", Some("VERSION=1.0.0")),
        ...
        assert_eq!(
            updater.create_version_content("1.2.3", Some("1.0.0")),
            "1.2.3\n"
        );
    }
